...
class MavenWebProjectScannerPluginTest {
    ...
    @Mock
    private MavenWebProjectDirectoryDescriptor webProjectDirectoryDescriptor;
    ...
    private final MavenWebProjectScannerPlugin plugin = new MavenWebProjectScannerPlugin();
    ...
    private void verifyScan() throws IOException {
        ...
        doReturn(store).when(scannerContext)
                .getStore();
        doReturn(webProjectDirectoryDescriptor).when(store)
                .addDescriptorType(projectDirectoryDescriptor, MavenWebProjectDirectoryDescriptor.class);
        doReturn(webapp).when(scanner)
                .scan(Mockito.argThat((ArgumentMatcher<File>) f -> f.equals(webappDir)), eq(WEBAPP), eq(DefaultScope.NONE));
        ...
        verify(store).addDescriptorType(projectDirectoryDescriptor, MavenWebProjectDirectoryDescriptor.class);
